...
class VGGnet_train(Network):
    def __init__(self, inputs, trainable=True):
        ...
        # self.data.shape is batch num of image, image height, image width and image channels
        # Because the input image is an RGB image, the number of image channels if always 3
        self.data = tf.placeholder(tf.float32, shape=[None, None, None, 3], name='data')
        # self.im_info is image_height, image_width, image scale factor
        self.im_info = tf.placeholder(tf.float32, shape=[None, 3], name='im_info')
        # self.keep_prob is used to dropout.
        ...
        self.trainable = trainable  # trainable is True
        self.setup()
